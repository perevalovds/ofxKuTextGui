...
struct KuUiComponent {
	...
	KuUiType index = KuUiType::VUndefined;
	bool visible = true;
	...
	bool need_save() { //we save only not-dummy, not-button, not-output vars (name doesn't start with "-")
		return (index != KuUiType::VDummy && !is_button() && (title().empty() || title()[0] != '-'));
	}

	bool is_string() {
		return (index == KuUiType::VString);
	}
	bool is_button() {
		return (index == KuUiType::VInt) && vint.is_button;
	}
	bool is_checkbox() {
		return (index == KuUiType::VInt) && vint.is_checkbox;
	}
	...
	void setTitle(const string& title) {
		if (index == KuUiType::VFloat)    vfloat.title = title;
		if (index == KuUiType::VInt)      vint.title = title;
		if (index == KuUiType::VString)   vstring.title = title;
		if (index == KuUiType::VStringList) vstringlist.title = title;
	}

	void setToggled(int t) {
		if (index == KuUiType::VInt) vint.setToggled(t);
	}

	void setValueInt(int v) {
		if (index == KuUiType::VInt) {
			vint.setValue(v);
		}
		else if (index == KuUiType::VStringList) {
			vstringlist.setValue(v);
		}
	}
	void setValue(const string& v) {
		if (index == KuUiType::VFloat) vfloat.setValue(ofToFloat(v));
		if (index == KuUiType::VInt) {
			if (is_checkbox()) {
		...
		if (index == KuUiType::VString) vstring.setValue(v);
		if (index == KuUiType::VStringList) vstringlist.setValueString(v);
	...
	void inc(int dir, int speed) {
		if (index == KuUiType::VFloat) vfloat.inc(dir, speed);
		if (index == KuUiType::VInt) vint.inc(dir, speed);
		if (index == KuUiType::VString) editStringValue(); //vstring.inc(dir,speed);
		if (index == KuUiType::VStringList) vstringlist.inc(dir, speed);
	}
	void editStringValue() {
		if (index == KuUiType::VString) vstring.editStringValue();
	}
	string value() {
		if (index == KuUiType::VFloat) return (fabs(*vfloat.var) >= 0.00001) ? ofToString(*vfloat.var) : "0";	//crop very small values
		if (index == KuUiType::VInt) return ofToString(*vint.var);
		if (index == KuUiType::VString) return *vstring.var;
		if (index == KuUiType::VStringList) return vstringlist.getValue();
		return "";
	}
	float valueNormalized() {
		if (index == KuUiType::VFloat) return (vfloat.minV != vfloat.maxV) ? ofMap(*vfloat.var, vfloat.minV, vfloat.maxV, 0, 1) : vfloat.minV;
		if (index == KuUiType::VInt) return (vint.minV != vint.maxV) ? ofMap(*vint.var, vint.minV, vint.maxV, 0, 1) : vint.minV;
		if (index == KuUiType::VStringList) return (vstringlist.minV != vstringlist.maxV) ? ofMap(*vstringlist.var, vstringlist.minV, vstringlist.maxV, 0, 1) : vstringlist.minV;

		return 0;
	}
	float normalizedToValue(float normalizedValue) {
		if (index == KuUiType::VFloat) return ofMap(normalizedValue, 0, 1, vfloat.minV, vfloat.maxV);
		if (index == KuUiType::VInt) return ofMap(normalizedValue, 0, 1, vint.minV, vint.maxV);
		if (index == KuUiType::VStringList) return ofMap(normalizedValue, 0, 1, vstringlist.minV, vstringlist.maxV);

		return 0;
	}

	int intValue() {
		if (index == KuUiType::VFloat) return 0;//int(*vfloat.var);
		if (index == KuUiType::VInt) return *vint.var;
		if (index == KuUiType::VString) return 0;
		if (index == KuUiType::VStringList) return *vstringlist.var;
		return 0;
	}
	float floatValue() {
		if (index == KuUiType::VFloat) return *vfloat.var;
		if (index == KuUiType::VInt) return *vint.var;
		if (index == KuUiType::VString) return 0;
		if (index == KuUiType::VStringList) return *vstringlist.var;
		return 0;
	...
	string name() {
		if (index == KuUiType::VFloat) return vfloat.name;
		if (index == KuUiType::VInt) return vint.name;
		if (index == KuUiType::VString) return vstring.name;
		if (index == KuUiType::VStringList) return vstringlist.name;
		return "";
	}
	string title() {
		if (index == KuUiType::VFloat) return vfloat.title;
		if (index == KuUiType::VInt) return vint.title;
		if (index == KuUiType::VString) return vstring.title;
		if (index == KuUiType::VStringList) return vstringlist.title;
		return "";
	}
	void reset() {
		if (index == KuUiType::VFloat) vfloat.reset();
		if (index == KuUiType::VInt) vint.reset();
		if (index == KuUiType::VString) vstring.reset();
		if (index == KuUiType::VStringList) vstringlist.reset();
	}
	void setTitles(vector<string>& titles) {
		if (index == KuUiType::VStringList) vstringlist.titles = titles;
	}
};
